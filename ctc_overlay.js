	    		 ...
	    		 var errorCount =0; 
		    	  ...
		    	  ctcOverlayViewer.objectToArray(unloadedGalImg).map((imgSpan)=>{
						...
						thumbImage.decode().then(()=>{
							   ...
							   var styleRule = [['display','block'],['background','url('+imgUrl+')'],['height',gallerySpanHeight+'px']];
			  		          ...
			  		          //script to load image and margin of close button 
								 ...
								 ctcOverlayViewer.removeClass(['overlayContentloadingV'],closeBtn);
									...
									if(totalImageCount>=2){
										...
										activeGallerySpan[0].scrollIntoView({behavior: "smooth", block: "center"});
											
										if(activeGallerySpan[0] !== undefined && activeGallerySpan[0].getAttribute("data-gal-unloaded-v") !== null){
										...
										else if(activeGallerySpan[0] !== undefined){
												
											activeGallerySpan[0].classList.add('ctcOverlayThumbGalleryActiveV');
										...
										if(imageNumberToLoad-1 >= 0  && imageNumberToLoad+1 < totalImageCount){
											...
											overlayImgContainer.appendChild(ctcOverlayViewer.addElemClass(['ctcGalleryRightNavV'],
											       ...
											       countAndCurrent.innerHTML = 'Image '+(imageNumberToLoad+1)+' of '+totalImageCount;
