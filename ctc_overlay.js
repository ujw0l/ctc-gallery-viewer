	...
	class ctcOverlayViewer {
		...
		prepareOverlay(elem) {
			...
			if (overlayDiv === null) {
				...
				downloadButton.setAttribute('title', 'Save Image');
		...
		static loadOverlayImages(currentImageNumber, resize) {
			...
			image.addEventListener('load', function () {
				...
				if (totalImageCount >= 2) {
					...
					if (activeGallerySpan !== undefined && activeGallerySpan.getAttribute("data-gal-unloaded-v") !== null) {
						var elemStyle = [
							["display", "block"],
					...
					} else if (activeGallerySpan !== undefined) {
						...
						overlayImgContainer.appendChild(ctcOverlayViewer.addElemClass(['ctcGalleryRightNavV'],
							ctcOverlayViewer.setElemAttr([
								...
								ctcOverlayViewer.applyStyle([
									...
									['margin-right', document.getElementById('ctcOverlayDownloadButtonV').offsetWidth + 'px']
