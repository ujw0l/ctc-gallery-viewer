...
class ctcOverlayViewer {
	...
	*/
	...
	createOverlay(img, imgNum, gal, param2) {
		...
		imgTitleDiv.style = `z-index:195000;overflow:auto;position:fixed;text-align:center;height:${0.02 * overlayHeight}px;width:${opImgDim.width}px;bottom:1px;color:rgba(255,255,255,1);font-size:${0.015 * overlayHeight};overflow:auto;left:${(sideBarWid * overlayWidth) + (((alltImgWidth * overlayWidth) - opImgDim.width) / 2)}px;`;
	...
	*/
	createToolbar(overlayDivEl, gal, imgEl, imgNum, param2) {
		...
		if (undefined == toolbarDiv) {
			...
			toolbarDiv.style = `top:${(ovHeight / 2) - (0.077 * ovWidth)}px;float:right;right: 0px;display: inline-block;position: fixed;`;
			...
			firstImgBtn.addEventListener('mouseenter', e => e.target.style.fontWeight = 'bolder');
			firstImgBtn.addEventListener('mouseleave', e => e.target.style.fontWeight = '');
			...
			let slideShowBtn = document.createElement('div')
			slideShowBtn.id = 'gal-slide-show';
			slideShowBtn.style = btnStyle + `font-size:${0.011 * ovWidth};text-align:center;line-height:2.15;`;
			slideShowBtn.title = "Start slideshow";
			slideShowBtn.innerHTML = ' &#9658;';
			slideShowBtn.setAttribute('data-interval-id', '0');
			slideShowBtn.addEventListener('click', e => {
				if ('0' == e.target.getAttribute('data-interval-id')) {
					e.target.setAttribute('data-status', 'play');
					e.target.innerHTML = ' &#9658;||';
					e.target.title = 'Stop slideshow'
					let ssIntervalId = setInterval(() => toolbarDiv.querySelector('#gal-next-img').click(), 3000);
					e.target.setAttribute('data-interval-id', ssIntervalId);
				} else {
					e.target.innerHTML = ' &#9658;';
					e.target.title = 'Start slideshow';
					clearInterval(parseInt(e.target.getAttribute('data-interval-id')))
					e.target.setAttribute('data-interval-id', '0');
				}

			})
			slideShowBtn.addEventListener('mouseenter', e => e.target.style.fontWeight = 'bolder');
			slideShowBtn.addEventListener('mouseleave', e => e.target.style.fontWeight = '');
			toolbarDiv.appendChild(slideShowBtn);
			...
			lastImgBtn.addEventListener('mouseenter', e => e.target.style.fontWeight = 'bolder');
			lastImgBtn.addEventListener('mouseleave', e => e.target.style.fontWeight = '');
